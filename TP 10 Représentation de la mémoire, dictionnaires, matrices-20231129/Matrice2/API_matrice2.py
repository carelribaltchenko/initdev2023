...
def matrice(nb_lignes, nb_colonnes, valeur_par_defaut):
    ...
    for i in range(nb_lignes):
        ...
        for j in range(nb_colonnes):
            matrice[i].append(valeur_par_defaut)
    return matrice
...
def get_diagonale_principale(la_matrice):
    if get_nb_lignes(la_matrice)!=get_nb_colonnes(la_matrice):
        return None
    res=[] 
    for i in range(get_nb_colonnes(la_matrice)):
        res.append(la_matrice[i][i])
    return res

print (get_diagonale_principale([[1,2,3],[4,5,6],[7,8,9]]))

def get_diagonale_secondaire(la_matrice):
    if get_nb_lignes(la_matrice)!=get_nb_colonnes(la_matrice):
        return None
    res=[] 
    for i in range(get_nb_colonnes(la_matrice)):
        res.append(la_matrice[i][get_nb_lignes(la_matrice)-i-1])
    return res

print (get_diagonale_secondaire([[1,2,3],[4,5,6],[7,8,9]]))