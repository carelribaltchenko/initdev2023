...
def init(nom_fichier="/home/iut45/Etudiants/o22301884/init-dev/initdev_git/initdev2023/TP12 plus court chemin dans une grille-20231215/labyrinthe//labyrinthe1.txt"):
    ...
    """
    le_plateau=matrice.charge_matrice(nom_fichier)
    matrice.set_valeur(le_plateau, 0,0,2)
    matrice.set_valeur(le_plateau, matrice.get_nombre_de_lignes(le_plateau)-1, matrice.get_nombre_de_colonnes(le_plateau)-1, 3)
    return le_plateau
...
def est_sur_le_plateau(le_plateau, position):
    ...
    """
    return not (position[0]>=matrice.get_nombre_de_lignes(le_plateau) or position[0]<0 or position[1]<0 or position[1]>=matrice.get_nombre_de_colonnes(le_plateau))
...
def get(le_plateau, position):
    ...
    if est_sur_le_plateau(le_plateau, position):
        return matrice.get_valeur(le_plateau, position[0], position[1])
    return None
...
def est_la_sortie(le_plateau, position):
    ...
    """
    return position[0]==matrice.get_nombre_de_lignes(le_plateau)-1 and position[1]==matrice.get_nombre_de_colonnes(le_plateau)-1
...
def deplace_personnage(le_plateau, personnage, direction):
    ...
    """
    cases_voisines=voisins(le_plateau, personnage)
    if direction == EST :
        if (personnage[0], personnage[1]+1) in cases_voisines:
            matrice.set_valeur(le_plateau, personnage[0], personnage[1], 0)
            matrice.set_valeur(le_plateau, personnage[0], personnage[1]+1, 2)
            return (personnage[0], personnage[1]+1)
    if direction == NORD :
        if (personnage[0]-1, personnage[1]) in cases_voisines:
            matrice.set_valeur(le_plateau, personnage[0], personnage[1], 0)
            matrice.set_valeur(le_plateau, personnage[0]-1, personnage[1], 2)
...
def voisins(le_plateau, position):
    ...
    """
    res=set()
    if est_sur_le_plateau(le_plateau, (position[0]-1, position[1])) and (get(le_plateau, (position[0]-1, position[1]))==0 or get(le_plateau, (position[0]-1, position[1]))==3) :
        res.add(((position[0]-1, position[1]))) 
    if est_sur_le_plateau(le_plateau, (position[0]+1, position[1])) and (get(le_plateau, (position[0]+1, position[1]))==0 or get(le_plateau, (position[0]+1, position[1]))==3) :
        res.add(((position[0]+1, position[1]))) 
    if est_sur_le_plateau(le_plateau, (position[0], position[1]-1)) and (get(le_plateau, (position[0], position[1]-1))==0 or get(le_plateau, (position[0], position[1]-1))==3) :
...
def fabrique_le_calque(le_plateau, position_depart):
    ...
    """
    mat=le_plateau.copy()
    for i in range(matrice.get_nombre_de_lignes(le_plateau)-1):
        for j in range(matrice.get_nombre_de_colonnes(le_plateau)-1):
            if le_plateau[i][j]!=0:
                matrice.set_valeur(mat,i,j , None)
    matrice.set_valeur(mat, position_depart[0], position_depart[1], 0)
    ens=set()
    ens=ens.union(voisins(mat,position_depart))
    pos=0
    print(ens)
    while len(ens)!=0:
        futur=set()
        for pos_voisin in ens:
            matrice.set_valeur(mat,  pos_voisin[0],pos_voisin[1], pos+1)
            futur=futur.union(voisins(mat, pos_voisin))
        pos+=1
        ens=futur
    print(mat)
    return mat
...
le_plateau = init()
le_calque = fabrique_le_calque(le_plateau, (4, 2))
